...
type GearItem = { id: string; category: 'shoes' | 'bag' | 'accessory'; brand: string; model: string; details: string; terrain?: 'road' | 'trail' | 'hyrox' | 'mixed'; use?: string; confidence?: 'automatic' | 'confirm' }
...
const normalizeProfile = (saved: Omit<Partial<AthleteProfile>, 'gear'> & { age?: number; gear?: Array<Partial<GearItem> & { name?: string }> }): AthleteProfile => ({
  ...
  gear: (saved.gear ?? []).filter(item => item.category !== ('hydration' as GearItem['category'])).map(item => {
    ...
    return { id: item.id || createId(), category, brand, model, details: item.details ?? '', ...(category === 'shoes' ? inferShoe(brand, model) : {}) }
  }),
...
function UniversePage() {
  ...
  const addShoe = () => { if (!shoeBrand.trim() || !shoeModel.trim()) return; const gear = [...draft.gear, { id: createId(), category: 'shoes' as const, brand: shoeBrand.trim(), model: shoeModel.trim(), details: '', ...inferShoe(shoeBrand, shoeModel) }]; persistGear(gear); setShoeBrand(''); setShoeModel('') }
  ...
  const addOtherGear = () => { if (!otherModel.trim()) return; const gear = [...draft.gear, { id: createId(), category: otherCategory, brand: otherBrand.trim(), model: otherModel.trim(), details: otherDetails.trim() }]; persistGear(gear); setOtherBrand(''); setOtherModel(''); setOtherDetails('') }
  ...
  return <section className="universe-page universe-overview">
    ...
    <AnimatePresence>{panel && <motion.div className="dialog-backdrop universe-backdrop" role="presentation" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={event => { if (event.target === event.currentTarget) setPanel(null) }}><motion.section className="universe-sheet" role="dialog" aria-modal="true" aria-labelledby="universe-sheet-title" initial={{ y: 36, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 28, opacity: 0 }}><header><div><span>MON UNIVERS</span><h2 id="universe-sheet-title">{panel === 'profile' ? 'Modifier mon profil' : panel === 'profileView' ? 'Mon profil sportif' : panel === 'nutrition' ? 'Préférences nutritionnelles' : panel === 'shoes' ? 'Mes chaussures' : panel === 'hydration' ? 'Mon hydratation' : 'Sacs et accessoires'}</h2></div><button type="button" aria-label="Fermer" onClick={() => setPanel(null)}><X size={20} /></button></header>
      <div className="universe-sheet-body">
        ...
        {panel === 'shoes' && <><div className="gear-list">{shoes.length ? shoes.map(item => <article key={item.id}><div><span>{terrainLabels[item.terrain ?? 'mixed']} · {item.use}</span><strong>{item.brand} {item.model}</strong><small>{item.confidence === 'automatic' ? 'Caractéristiques reconnues automatiquement' : 'Caractéristiques à confirmer'}</small></div><button type="button" aria-label={`Supprimer ${item.brand} ${item.model}`} onClick={() => persistGear(draft.gear.filter(gear => gear.id !== item.id))}><Trash size={16} /></button></article>) : <p>Aucune chaussure enregistrée.</p>}</div><div className="gear-add shoe-add"><input placeholder="Marque, ex. Salomon" value={shoeBrand} onChange={event => setShoeBrand(event.target.value)} /><input placeholder="Modèle, ex. Genesis" value={shoeModel} onChange={event => setShoeModel(event.target.value)} /><button type="button" onClick={addShoe}><Plus size={17} /> Analyser et ajouter</button></div></>}
        {panel === 'hydration' && <><div className="gear-list">{draft.hydration.length ? draft.hydration.map(item => <article key={item.id}><div><span>{hydrationLabels[item.type]}</span><strong>{item.capacity >= 1000 ? `${(item.capacity / 1000).toLocaleString('fr-FR')} L` : `${item.capacity} ml`} × {item.quantity}</strong><small>Capacité totale : {item.capacity * item.quantity >= 1000 ? `${(item.capacity * item.quantity / 1000).toLocaleString('fr-FR')} L` : `${item.capacity * item.quantity} ml`}</small></div><button type="button" aria-label={`Supprimer ${hydrationLabels[item.type]}`} onClick={() => persistHydration(draft.hydration.filter(container => container.id !== item.id))}><Trash size={16} /></button></article>) : <p>Aucun contenant enregistré.</p>}</div><div className="gear-add hydration-add"><label className="gear-unit-field"><span>Type</span><select aria-label="Type de contenant" value={hydrationType} onChange={event => setHydrationType(event.target.value as HydrationItem['type'])}><option value="flask">Flasque</option><option value="bottle">Gourde</option><option value="bladder">Poche d’hydratation</option></select></label><label className="gear-unit-field"><span>Contenance</span><span><input aria-label="Contenance en millilitres" type="number" min="100" max="5000" step="50" value={hydrationCapacity} onChange={event => setHydrationCapacity(Number(event.target.value))} /><i>ml</i></span></label><label className="gear-unit-field"><span>Quantité</span><input aria-label="Quantité" type="number" min="1" max="20" value={hydrationQuantity} onChange={event => setHydrationQuantity(Number(event.target.value))} /></label><button type="button" onClick={addHydration}><Plus size={17} /> Ajouter</button></div></>}
        {panel === 'gear' && <><div className="gear-list">{[...bags, ...accessories].length ? [...bags, ...accessories].map(item => <article key={item.id}><div><span>{item.category === 'bag' ? 'Sac' : 'Accessoire'}</span><strong>{item.brand} {item.model}</strong><small>{item.details || 'Aucun détail'}</small></div><button type="button" aria-label={`Supprimer ${item.model}`} onClick={() => persistGear(draft.gear.filter(gear => gear.id !== item.id))}><Trash size={16} /></button></article>) : <p>Aucun sac ou accessoire enregistré.</p>}</div><div className="gear-add"><select value={otherCategory} onChange={event => setOtherCategory(event.target.value as GearItem['category'])}><option value="bag">Sac</option><option value="accessory">Accessoire</option></select><input placeholder="Marque" value={otherBrand} onChange={event => setOtherBrand(event.target.value)} /><input placeholder="Modèle" value={otherModel} onChange={event => setOtherModel(event.target.value)} /><input placeholder="Volume, usage…" value={otherDetails} onChange={event => setOtherDetails(event.target.value)} /><button type="button" onClick={addOtherGear}><Plus size={17} /> Ajouter</button></div></>}
