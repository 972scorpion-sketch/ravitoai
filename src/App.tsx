...
function NutritionEstimate({ outing, onAgenda, onNutrition }: { outing: PlannedOuting; onAgenda: () => void; onNutrition: () => void }) {
  ...
  for (const container of availableContainers) {
    if (packedWater >= water) break
    suggestedContainers.push(container)
    packedWater += container.capacity
  }
  const hydrationLabel = suggestedContainers.length
    ? suggestedContainers.map(item => `${item.type === 'flask' ? 'Flasque' : item.type === 'bottle' ? 'Gourde' : 'Poche'} ${item.capacity >= 1000 ? `${item.capacity / 1000} L` : `${item.capacity} ml`}`).join(' + ')
    : shortEasy ? 'Selon la soif et la météo' : profile.hydration.length ? 'Capacité insuffisante' : 'À renseigner dans Mon Univers'
  return <div className="nutrition-estimate"><div className="estimate-success"><Sparkle size={24} weight="duotone" /><div><strong>{outing.title} est dans ton agenda</strong><span>{formatDate(outing)} · {outing.time}</span></div></div><div className="estimate-grid"><article><span>Durée probable</span><strong>{formatDuration(outing.duration)}</strong></article><article><span>Hydratation</span><strong>{shortEasy ? 'Selon la soif' : `≈ ${water} ml`}</strong></article><article><span>Glucides pendant</span><strong>{carbs ? `≈ ${carbs} g` : 'Pas nécessaires'}</strong></article><article><span>Sodium dédié</span><strong>{shortEasy ? 'Pas nécessaire' : 'À affiner selon conditions'}</strong></article></div><section className="estimate-packing"><span>APRÈS L’AJOUT · SUGGESTION À EMPORTER</span><strong>{hydrationLabel}</strong>{suggestedContainers.length > 0 && <small>Capacité disponible proposée : {packedWater} ml</small>}<p>{shortEasy ? 'Pour cette séance courte et facile, eau, glucides et sodium ne sont pas imposés. Adapte selon la chaleur, ta soif et le moment du dernier repas.' : 'La quantité sera affinée dans Nutrition avec la météo, l’intensité et ta transpiration.'}</p></section><p className="estimate-note">Ces repères sont des estimations générales, pas des prescriptions médicales.</p><div className="estimate-actions"><Button variant="ghost" onClick={onAgenda}>Retour à l’accueil</Button><Button onClick={onNutrition}><Drop size={17} /> Affiner ma nutrition</Button></div></div>
}
...
function UniversePage() {
  ...
  const [shoeBrand, setShoeBrand] = useState('')
  const [shoeModel, setShoeModel] = useState('')
  const [shoeEditorOpen, setShoeEditorOpen] = useState(false)
  ...
  const [hydrationEditorOpen, setHydrationEditorOpen] = useState(false)
  const [otherCategory, setOtherCategory] = useState<GearItem['category']>('bag')
  const [otherBrand, setOtherBrand] = useState('')
  const [otherModel, setOtherModel] = useState('')
  const [otherDetails, setOtherDetails] = useState('')
  ...
  const startAddShoe = () => { resetShoe(); setShoeEditorOpen(true) }
  const editShoe = (item: GearItem) => { setShoeEditingId(item.id); setShoeBrand(item.brand); setShoeModel(item.model); setShoeMileage(item.mileage ?? 0); setShoeTerrain(item.terrain ?? 'mixed'); setShoeUse(item.use ?? 'Polyvalent'); setShoeCharacteristics(item.characteristics ?? ''); setShoeEditorOpen(true) }
  const closeShoeEditor = () => { resetShoe(); setShoeEditorOpen(false) }
  const addShoe = () => { if (!shoeBrand.trim() || !shoeModel.trim()) return; const inferred = inferShoe(shoeBrand, shoeModel); const shoe: GearItem = { id: shoeEditingId ?? createId(), category: 'shoes', brand: shoeBrand.trim(), model: shoeModel.trim(), details: '', mileage: Math.max(0, shoeMileage), terrain: shoeTerrain, use: shoeUse.trim() || inferred.use, characteristics: shoeCharacteristics.trim(), confidence: inferred.confidence }; const gear = shoeEditingId ? draft.gear.map(item => item.id === shoeEditingId ? shoe : item) : [...draft.gear, shoe]; persistGear(gear); closeShoeEditor() }
  const resetHydration = () => { setHydrationEditingId(null); setHydrationType('flask'); setHydrationCapacity(500); setHydrationQuantity(1) }
  const startAddHydration = () => { resetHydration(); setHydrationEditorOpen(true) }
  const editHydration = (item: HydrationItem) => { setHydrationEditingId(item.id); setHydrationType(item.type); setHydrationCapacity(item.capacity); setHydrationQuantity(item.quantity); setHydrationEditorOpen(true) }
  const closeHydrationEditor = () => { resetHydration(); setHydrationEditorOpen(false) }
  const addHydration = () => { if (hydrationCapacity < 100 || hydrationQuantity < 1) return; if (hydrationEditingId) { persistHydration(draft.hydration.map(item => item.id === hydrationEditingId ? { ...item, type: hydrationType, capacity: hydrationCapacity, quantity: hydrationQuantity } : item)); closeHydrationEditor(); return } const existing = draft.hydration.find(item => item.type === hydrationType && item.capacity === hydrationCapacity); const hydration = existing ? draft.hydration.map(item => item.id === existing.id ? { ...item, quantity: item.quantity + hydrationQuantity } : item) : [...draft.hydration, { id: createId(), type: hydrationType, capacity: hydrationCapacity, quantity: hydrationQuantity }]; persistHydration(hydration); closeHydrationEditor() }
  const resetOtherGear = () => { setOtherEditingId(null); setOtherCategory('bag'); setOtherBrand(''); setOtherModel(''); setOtherDetails('') }
  const startAddOtherGear = () => { resetOtherGear(); setOtherEditorOpen(true) }
  const editOtherGear = (item: GearItem) => { setOtherEditingId(item.id); setOtherCategory(item.category); setOtherBrand(item.brand); setOtherModel(item.model); setOtherDetails(item.details); setOtherEditorOpen(true) }
  const closeOtherEditor = () => { resetOtherGear(); setOtherEditorOpen(false) }
  const addOtherGear = () => { if (!otherModel.trim()) return; const item: GearItem = { id: otherEditingId ?? createId(), category: otherCategory, brand: otherBrand.trim(), model: otherModel.trim(), details: otherDetails.trim() }; const gear = otherEditingId ? draft.gear.map(gearItem => gearItem.id === otherEditingId ? item : gearItem) : [...draft.gear, item]; persistGear(gear); closeOtherEditor() }
  const openPanel = (next: UniversePanel) => { setDraft(profile); setSaved(false); if (next === 'shoes') closeShoeEditor(); if (next === 'hydration') closeHydrationEditor(); if (next === 'gear') closeOtherEditor(); setPanel(next) }
  ...
  return <section className="universe-page universe-overview">
    ...
    <AnimatePresence>{panel && <motion.div className="dialog-backdrop universe-backdrop" role="presentation" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={event => { if (event.target === event.currentTarget) setPanel(null) }}><motion.section className="universe-sheet" role="dialog" aria-modal="true" aria-labelledby="universe-sheet-title" initial={{ y: 36, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 28, opacity: 0 }}><header><div><span>MON UNIVERS</span><h2 id="universe-sheet-title">{panel === 'profile' ? 'Modifier mon profil' : panel === 'profileView' ? 'Mon profil sportif' : panel === 'nutrition' ? 'Préférences nutritionnelles' : panel === 'shoes' ? 'Mes chaussures' : panel === 'hydration' ? 'Mon hydratation' : 'Sacs et accessoires'}</h2></div><button type="button" aria-label="Fermer" onClick={() => setPanel(null)}><X size={20} /></button></header>
      <div className="universe-sheet-body">
        ...
        {panel === 'shoes' && (shoeEditorOpen ? <div className="shoe-editor shoe-editor--separate"><div className="shoe-editor-title"><div><span>{shoeEditingId ? 'MODIFIER LA PAIRE' : 'NOUVELLE PAIRE'}</span><strong>{shoeEditingId ? `${shoeBrand} ${shoeModel}` : 'Ajouter une chaussure'}</strong></div><button type="button" onClick={closeShoeEditor}>Retour à la liste</button></div><div className="gear-add shoe-add detailed"><label>Marque<input autoFocus placeholder="Ex. Salomon" value={shoeBrand} onChange={event => setShoeBrand(event.target.value)} /></label><label>Modèle<input placeholder="Ex. Genesis" value={shoeModel} onBlur={analyzeShoe} onChange={event => setShoeModel(event.target.value)} /></label><label>Kilométrage<input type="number" min="0" step="1" value={shoeMileage} onChange={event => setShoeMileage(Number(event.target.value))} /></label><label>Terrain<select value={shoeTerrain} onChange={event => setShoeTerrain(event.target.value as NonNullable<GearItem['terrain']>)}><option value="road">Route</option><option value="trail">Trail</option><option value="hyrox">Hyrox</option><option value="mixed">Mixte</option></select></label><label className="field-wide">Usage principal<input placeholder="Entraînement quotidien, compétition…" value={shoeUse} onChange={event => setShoeUse(event.target.value)} /></label><label className="field-wide">Caractéristiques connues<textarea rows={3} placeholder="Amorti, stabilité, dynamisme, drop…" value={shoeCharacteristics} onChange={event => setShoeCharacteristics(event.target.value)} /></label><button type="button" className="shoe-analyze" onClick={analyzeShoe}><Sparkle size={17} /> Analyser marque et modèle</button><button type="button" onClick={addShoe}>{shoeEditingId ? <Check size={17} /> : <Plus size={17} />} {shoeEditingId ? 'Enregistrer la paire' : 'Ajouter la paire'}</button></div><p className="shoe-source-note">Le kilométrage pourra être synchronisé avec Strava plus tard. Les informations restent modifiables manuellement.</p></div> : <div className="shoe-list-view"><div className="gear-list shoe-list">{shoes.length ? shoes.map(item => <article key={item.id}><div><span>{terrainLabels[item.terrain ?? 'mixed']} · {item.use ?? 'Polyvalent'}</span><strong>{item.brand} {item.model}</strong><small>{item.mileage ?? 0} km · {item.characteristics || (item.confidence === 'automatic' ? 'Caractéristiques reconnues automatiquement' : 'Caractéristiques à compléter')}</small>{(item.mileage ?? 0) >= 700 && <small className="shoe-wear-warning"><WarningCircle size={13} /> Usure à surveiller</small>}</div><div className="gear-item-actions"><button type="button" aria-label={`Modifier ${item.brand} ${item.model}`} onClick={() => editShoe(item)}><PencilSimple size={16} /></button><button type="button" aria-label={`Supprimer ${item.brand} ${item.model}`} onClick={() => persistGear(draft.gear.filter(gear => gear.id !== item.id))}><Trash size={16} /></button></div></article>) : <p>Aucune chaussure enregistrée.</p>}</div><Button type="button" className="shoe-add-global" onClick={startAddShoe}><Plus size={17} /> Ajouter une chaussure</Button></div>)}
        {panel === 'hydration' && (hydrationEditorOpen ? <div className="gear-editor-view"><div className="shoe-editor-title"><div><span>{hydrationEditingId ? 'MODIFIER LE CONTENANT' : 'NOUVEAU CONTENANT'}</span><strong>{hydrationEditingId ? hydrationLabels[hydrationType] : 'Ajouter une hydratation'}</strong></div><button type="button" onClick={closeHydrationEditor}>Retour à la liste</button></div><div className="gear-add hydration-add hydration-add--separate"><label className="gear-unit-field"><span>Type</span><select aria-label="Type de contenant" value={hydrationType} onChange={event => setHydrationType(event.target.value as HydrationItem['type'])}><option value="flask">Flasque</option><option value="bottle">Gourde</option><option value="bladder">Poche d’hydratation</option></select></label><label className="gear-unit-field"><span>Contenance</span><span><input aria-label="Contenance en millilitres" type="number" min="100" max="5000" step="50" value={hydrationCapacity} onChange={event => setHydrationCapacity(Number(event.target.value))} /><i>ml</i></span></label><label className="gear-unit-field"><span>Quantité</span><input aria-label="Quantité" type="number" min="1" max="20" value={hydrationQuantity} onChange={event => setHydrationQuantity(Number(event.target.value))} /></label><button type="button" onClick={addHydration}>{hydrationEditingId ? <Check size={17} /> : <Plus size={17} />} {hydrationEditingId ? 'Enregistrer le contenant' : 'Ajouter le contenant'}</button></div></div> : <div className="gear-list-view"><div className="gear-list">{draft.hydration.length ? draft.hydration.map(item => <article key={item.id}><div><span>{hydrationLabels[item.type]}</span><strong>{item.capacity >= 1000 ? `${(item.capacity / 1000).toLocaleString('fr-FR')} L` : `${item.capacity} ml`} × {item.quantity}</strong><small>Capacité totale : {item.capacity * item.quantity >= 1000 ? `${(item.capacity * item.quantity / 1000).toLocaleString('fr-FR')} L` : `${item.capacity * item.quantity} ml`}</small></div><div className="gear-item-actions"><button type="button" aria-label={`Modifier ${hydrationLabels[item.type]}`} onClick={() => editHydration(item)}><PencilSimple size={16} /></button><button type="button" aria-label={`Supprimer ${hydrationLabels[item.type]}`} onClick={() => persistHydration(draft.hydration.filter(container => container.id !== item.id))}><Trash size={16} /></button></div></article>) : <p>Aucun contenant enregistré.</p>}</div><Button type="button" className="shoe-add-global" onClick={startAddHydration}><Plus size={17} /> Ajouter une hydratation</Button></div>)}
        {panel === 'gear' && (otherEditorOpen ? <div className="gear-editor-view"><div className="shoe-editor-title"><div><span>{otherEditingId ? 'MODIFIER LE MATÉRIEL' : 'NOUVEAU MATÉRIEL'}</span><strong>{otherEditingId ? `${otherBrand} ${otherModel}` : 'Ajouter un sac ou accessoire'}</strong></div><button type="button" onClick={closeOtherEditor}>Retour à la liste</button></div><div className="gear-add gear-add--separate"><label>Catégorie<select value={otherCategory} onChange={event => setOtherCategory(event.target.value as GearItem['category'])}><option value="bag">Sac</option><option value="accessory">Accessoire</option></select></label><label>Marque<input autoFocus placeholder="Marque" value={otherBrand} onChange={event => setOtherBrand(event.target.value)} /></label><label>Modèle<input placeholder="Modèle" value={otherModel} onChange={event => setOtherModel(event.target.value)} /></label><label className="field-wide">Détails<input placeholder="Volume, usage…" value={otherDetails} onChange={event => setOtherDetails(event.target.value)} /></label><button type="button" onClick={addOtherGear}>{otherEditingId ? <Check size={17} /> : <Plus size={17} />} {otherEditingId ? 'Enregistrer le matériel' : 'Ajouter le matériel'}</button></div></div> : <div className="gear-list-view"><div className="gear-list">{[...bags, ...accessories].length ? [...bags, ...accessories].map(item => <article key={item.id}><div><span>{item.category === 'bag' ? 'Sac' : 'Accessoire'}</span><strong>{item.brand} {item.model}</strong><small className={!item.details ? 'gear-missing' : ''}>{item.details || 'Informations à compléter'}</small></div><div className="gear-item-actions"><button type="button" aria-label={`Modifier ${item.model}`} onClick={() => editOtherGear(item)}><PencilSimple size={16} /></button><button type="button" aria-label={`Supprimer ${item.model}`} onClick={() => persistGear(draft.gear.filter(gear => gear.id !== item.id))}><Trash size={16} /></button></div></article>) : <p>Aucun sac ou accessoire enregistré.</p>}</div><Button type="button" className="shoe-add-global" onClick={startAddOtherGear}><Plus size={17} /> Ajouter un sac ou accessoire</Button></div>)}
      </div>{(panel === 'profile' || panel === 'nutrition') && <footer><Button variant="ghost" type="button" onClick={() => setPanel(null)}>Annuler</Button><Button className={saved ? 'saved' : ''} type="button" onClick={save}>{saved ? <Check size={17} /> : <Sparkle size={17} />} {saved ? 'Enregistré' : 'Enregistrer'}</Button></footer>}</motion.section></motion.div>}</AnimatePresence>
  </section>
}
...
function NutritionPage() {
  ...
  useEffect(() => {
    if (selected) setIntensity(selected.intensity ?? (selected.kind === 'race' ? 'hard' : 'moderate'))
  }, [outingId, selected?.intensity, selected?.kind])

  useEffect(() => {
    const outingIntensity = selected?.intensity ?? (selected?.kind === 'race' ? 'hard' : 'moderate')
    setIntensity(outingIntensity)
    if (!savedPlan) { setWeather('clear'); setTemperature(18); setSweat('average'); setDigestion('normal'); setProductSelection([]); setHasEstimate(false); setSaved(false); return }
    setWeather(savedPlan.weather); setTemperature(savedPlan.temperature); setSweat(savedPlan.sweat); setDigestion(savedPlan.digestion); setWaterPerHour(savedPlan.waterPerHour); setCarbsPerHour(savedPlan.carbsPerHour); setSodiumPerHour(savedPlan.sodiumPerHour); setProductSelection(savedPlan.productSelection ?? []); setHasEstimate(true); setSaved(true)
  }, [outingId, savedPlan?.updatedAt, selected?.intensity, selected?.kind])

  const hours = selected ? Math.max(.25, selected.duration.split(':').map(Number).reduce((total, value, index) => total + value / (index ? 60 : 1), 0)) : 0
  const caffeineTarget = Math.min(profile.caffeineLimit, Math.round(profile.weight * 3))
  const profileAllergens = profile.allergens.toLowerCase().split(',').map(item => item.trim()).filter(Boolean)
  const caffeineProduct = products.find(product => product.quantity > 0 && product.caffeine > 0 && product.caffeine <= caffeineTarget && !product.poorTolerance && !profileAllergens.some(allergen => product.allergens.toLowerCase().includes(allergen)) && hasCompleteComposition(product))
  const estimate = () => {
    ...
    carbs = shortEasy || hours < .75 ? 0 : Math.min(90, carbs)
    sodium = hours <= 1 ? 0 : Math.min(1200, Math.max(250, Math.round(sodium / 50) * 50))
  ...
  const warnings = useMemo(() => {
    const items: string[] = []
    const shortEasy = selected?.kind === 'training' && intensity === 'easy' && hours <= 1
    if (waterPerHour > 0 && waterPerHour < 300 && !shortEasy) items.push('Hydratation très faible : vérifie la température et ton niveau de transpiration.')
    ...
    return items
  }, [waterPerHour, carbsPerHour, sodiumPerHour, hours, temperature, sweat, selected, intensity, caffeineProduct, caffeineTarget])
  const timeline = useMemo(() => {
    ...
    if (!totalWater && !totalCarbs && !totalSodium) return steps
    const preCarbs = totalMinutes <= 75 && totalCarbs > 0 ? totalCarbs : 0
    if (preCarbs) steps.push({ minute: -10, water: 0, carbs: preCarbs, sodium: 0 })
    const preWater = totalMinutes >= 90 && totalWater >= 400 ? Math.min(200, totalWater) : 0
    if (preWater) steps.push({ minute: -15, water: preWater, carbs: 0, sodium: 0 })
    const moments = Array.from({ length: Math.max(0, Math.ceil(totalMinutes / 30) - 1) }, (_, index) => (index + 1) * 30).filter(minute => minute < totalMinutes)
    if (!moments.length) return steps
    const remainingWater = totalWater - preWater
    moments.forEach((minute, index) => {
      const isLast = index === moments.length - 1
      const previous = steps.filter(step => step.minute >= 0)
      const usedWater = previous.reduce((sum, step) => sum + step.water, 0)
      const usedCarbs = previous.reduce((sum, step) => sum + step.carbs, 0)
      const usedSodium = previous.reduce((sum, step) => sum + step.sodium, 0)
      steps.push({
        minute,
        water: isLast ? remainingWater - usedWater : Math.round(remainingWater / moments.length / 10) * 10,
        carbs: isLast ? totalCarbs - preCarbs - usedCarbs : Math.round((totalCarbs - preCarbs) / moments.length),
        sodium: isLast ? totalSodium - usedSodium : Math.round(totalSodium / moments.length / 10) * 10,
      })
    })
    return steps
